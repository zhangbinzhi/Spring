 ...
 */
var measureToolDraw; // global so we can remove it later
...
ol.control.MeasureTool.prototype.mapmeasure = function (typeSelect) {
    ...
    var measureTooltip;
    var map = this.getMap();
    if(measureToolDraw!=null){
        map.removeInteraction(measureToolDraw);
    }
    ...
    var formatLength = function (line) {
        ...
        if (typeSelect.check) {
            ...
            for (var i = 0, ii = coordinates.length - 1; i < ii; ++i) {
                ...
                length += ol.sphere.getDistance(c1, c2.this.sphereradius);
            }
        } else {
            var sourceProj = map.getView().getProjection();
    ...
    var formatArea = function (polygon) {
        if (typeSelect.check) {
            ...
            area = Math.abs(ol.sphere.getArea(coordinates, this.sphereradius));
        } else {
            var sourceProj = map.getView().getProjection();
            var geom = /** @type {ol.geom.Polygon} */(polygon.clone().transform(
                sourceProj, 'EPSG:3857'));
    ...
    function addInteraction() {
        ...
        measureToolDraw = new ol.interaction.Draw({
            source: source,
        ...
        map.addInteraction(measureToolDraw);
    }
